...
@ActionConfig(name = "微信账户审核下发")
public class TransferToWechatAccountService extends
		AbstractTransferService<TransferToWechatAccountServiceParams, TransferToWechatAccountServiceAttachment> {

	private static final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

	private static final Logger logger = LoggerFactory.getLogger(TransferToWechatAccountService.class);
	...
	public String getActionType() {
		return APIDefinition.TRANSFER_TO_WECHAT_ACCOUNT.name();
	}

	@Override
	protected Map<String, String> checkAndCountFeeBeforeTransfer(ChannelRelated channelWithContract,
			TransferToWechatAccountServiceParams params) {
...
//			throw new APIDockingException(APIDockingRetCodes.FIELD_FORMAT_ERROR.getCode(), params.getOpenId());
//		}
	...
	@Override
	protected ActionResult<TransferToWechatAccountServiceAttachment> transfer(String dealNo,
			TransferToWechatAccountServiceParams actionParams, Map<String, String> feeInfo,
		...
		userCommission.setAccount(actionParams.getOpenId());

		userCommission.setCalculationRates(feeInfo.get(AbstractTransferServiceParams.CALCULATION_RATES));
		userCommission.setCertId(actionParams.getCertificateNo());
		...
		userCommission.setRemark(actionParams.getRemark());

		userCommission.setDocumentType(1);

		userCommission.setMerchantId(feeInfo.get("merchantId"));
		userCommission.setOrderNo(dealNo);
		userCommission.setPhoneNo(actionParams.getMobileNo());
		
		userCommission.setPayType(3);
		...
		if (ApiReturnCode.SUCCESS.getCode().equals(retCode)) {
			TransferToWechatAccountServiceAttachment attachment = new TransferToWechatAccountServiceAttachment();
			...
			return new ActionResult<TransferToWechatAccountServiceAttachment>(attachment);
