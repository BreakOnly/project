...
public class Sign {
     ...
     * @return
     */
	public static String signByPriKey(byte[] bytes,String privateKey){
		String b64SignMsg =null;
		try {
			b64SignMsg =RsaUtil.sign(bytes, privateKey,"SHA1withRSA");
	...
	public static boolean verfySignByPubKey(String srcMessage,String resMessage,String publicKey){
		try {
			boolean verify  =RsaUtil.verify(srcMessage.getBytes("UTF-8"), publicKey, "SHA1withRSA", resMessage);
			if (!verify) {
				return false;
			}
		} catch (Exception e) {
